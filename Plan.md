...
4. **E2E без транка — loopback**: тестовий режим набору (env
   ...
   `loopback/9999`): `originate loopback/9999 &socket(...)` ганяє повний
   сценарій §6 усередині FreeSWITCH без жодного зовнішнього дзвінка; DTMF
   симулюється `uuid_send_dtmf <uuid> 1`; перевіряються переходи статусів у
   БД (`answered`, `transferred`-гілка до моменту bridge, `optout`...).
