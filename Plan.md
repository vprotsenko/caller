...
- **Кампанія**: повідомлення + голос + прев'ю; IVR-редактор — рекурсивне
  дерево рівнів (§5): рядок опції = клавіша + дія + поля дії, дія «підменю»
  розгортає вкладений рівень зі своїм текстом/анонсом; плейсхолдер анонсу
  дзеркалить серверний автотекст; таймаут/повтори — глобальні поля; список
  номерів; `max_concurrent`; живий прогрес (полінг `/status` 1.5 с) з
  лічильниками за всіма статусами §4.
...
(`operator|replay|menu|play|back|optout|hangup`); для `play` — `text` і
